...
const arbState: fc.Arbitrary<FullMachineState> = fc.record({
  state: fc.constantFrom("IDLE", "CONNECTING", "CALLER_WAITING", "CALLEE_WAITING", "NEGOTIATING", "CONNECTED"),
...
const arbEvent: fc.Arbitrary<MachineEvent> = fc.oneof(
  ...
  fc.record({ type: fc.constant("offer"),         offer:      fc.record({ type: fc.constant("offer" as const),  sdp: fc.string() }) }),
  fc.record({ type: fc.constant("answer"),        answer:     fc.record({ type: fc.constant("answer" as const), sdp: fc.string() }) }),
  fc.record({ type: fc.constant("ice-candidate"), candidate:  fc.record({ candidate: fc.string() }) }),
...
describe("Signaling Machine Invariants", () => {
  it("never crashes on any (state, event) pair", () => {
    fc.assert(fc.property(arbState, arbEvent, (state, event) => {
      expect(() => transition(state, event)).not.toThrow()
    }))
  })

  it("CONNECTED is only reachable via ice-connected", () => {
    fc.assert(fc.property(arbState, arbEventSequence, (initial, events) => {
      let prev = initial
  ...
  it("after onclose, a subsequent enter/offer produces NEGOTIATING (F5 bug regression)", () => {
    fc.assert(fc.property(arbRole, (role) => {
      ...
      const followUp: MachineEvent = role === "caller"
        ? { type: "enter" }
...
function isCoherent(state: FullMachineState): boolean {
  ...
  if ((state.state === "NEGOTIATING" || state.state === "CONNECTED") && state.role !== null) return true;
  return false;
}
  ...
  it("SETUP_PC coupling: emitted ⇔ onopen or peer-reconnected(callee), next.state matches role", () => {
    fc.assert(fc.property(arbState, arbEvent, (state, event) => {
      ...
      if (setup && setup.type === "SETUP_PC") {
        const validTrigger = event.type === "onopen" ||
  ...
  it("two-machine: premature offer (onnegotiationneeded before enter) — answer must not be WARNed in CALLER_WAITING", () => {
    ...
    const { next: callerAfterAnswer, effects } = transition(
      caller,
  ...
  it("after peer-reconnected as callee, a subsequent offer produces NEGOTIATING", () => {
    ...
    const { next } = transition(afterReconnect, { type: "offer", offer: { type: "offer", sdp: "v=0" } })
    expect(next.state).toBe("NEGOTIATING")
  })
})
