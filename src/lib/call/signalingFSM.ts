...
function handleEnter(full: FullMachineState): { next: FullMachineState, effects: Effect[] } {
  ...
  return { next: full, effects: [{ type: "WARN", message: `unhandled event: enter in ${full.state}` }] }
}
...
function handleOnclose(full: FullMachineState): { next: FullMachineState, effects: Effect[] } {
  ...
  return { next: full, effects: [{ type: "WARN", message: `unhandled event: onclose in ${full.state}` }] }
}

function handleOffer(full: FullMachineState, event: { type: "offer", offer: RTCSessionDescriptionInit }): { next: FullMachineState, effects: Effect[] } {
  if (full.state === "CALLEE_WAITING") {
    return { next: { ...full, state: "NEGOTIATING" }, effects: [{ type: "HANDLE_OFFER", offer: event.offer }] }
  } else if (full.state === "NEGOTIATING" || full.state === "CONNECTED") {
    return { next: full, effects: [{ type: "HANDLE_OFFER", offer: event.offer }] }
  }
  return { next: full, effects: [{ type: "WARN", message: `unhandled event: offer in ${full.state}` }] }
}

function handleAnswer(full: FullMachineState, event: { type: "answer", answer: RTCSessionDescriptionInit }): { next: FullMachineState, effects: Effect[] } {
  if (full.state === "NEGOTIATING" || full.state === "CONNECTED") {
    return { next: full, effects: [{ type: "HANDLE_ANSWER", answer: event.answer }] }
  }
  if (full.state === "CALLER_WAITING") {
    return { next: { ...full, state: "NEGOTIATING" }, effects: [{ type: "HANDLE_ANSWER", answer: event.answer }] }
  }
  return { next: full, effects: [{ type: "WARN", message: `unhandled event: answer in ${full.state}` }] }
}

function handleIceCandidate(full: FullMachineState, event: { type: "ice-candidate", candidate: RTCIceCandidateInit }): { next: FullMachineState, effects: Effect[] } {
  if (full.state === "NEGOTIATING" || full.state === "CONNECTED") {
    return { next: full, effects: [{ type: "HANDLE_ICE_CANDIDATE", candidate: event.candidate }] }
  }
  return { next: full, effects: [{ type: "WARN", message: `unhandled event: ice-candidate in ${full.state}` }] }
}
...
function handleIceConnected(full: FullMachineState): { next: FullMachineState, effects: Effect[] } {
  ...
  return { next: full, effects: [{ type: "WARN", message: `unhandled event: ice-connected in ${full.state}` }] }
}
...
function handleIceFailed(full: FullMachineState): { next: FullMachineState, effects: Effect[] } {
  if (full.state === "NEGOTIATING" || full.state === "CONNECTED") {
    if (full.role === "caller") {
      ...
      return { next: full, effects: [{ type: "WARN", message: `unhandled event: ice-failed in ${full.state}` }] }
    }
  }
  return { next: full, effects: [{ type: "WARN", message: `unhandled event: ice-failed in ${full.state}` }] }
}
...
export function transition(
  ...
  switch (event.type) {
    case "onopen":           return handleOnopen(full, event)
    case "enter":            return handleEnter(full)
    case "peer-reconnected": return handlePeerReconnected(full)
    case "onclose":          return handleOnclose(full)
    case "ping":             return { next: full, effects: [{ type: "SEND_WS", msg: { type: "pong" } }] }
    case "offer":            return handleOffer(full, event)
    case "answer":           return handleAnswer(full, event)
    case "ice-candidate":    return handleIceCandidate(full, event)
    case "ice-connected":    return handleIceConnected(full)
...
export function toMachineEvent(msg: ReceivedMessage): MachineEvent {
  switch (msg.type) {
    case "onopen":           return { type: "onopen", role: msg.role, reconnect: msg.reconnect }
    case "enter":            return { type: "enter" }
    case "peer-reconnected": return { type: "peer-reconnected" }
    case "onclose":          return { type: "onclose", message: msg.message }
    case "ping":             return { type: "ping" }
    case "offer":            return { type: "offer", offer: msg.offer }
    case "answer":           return { type: "answer", answer: msg.answer }
